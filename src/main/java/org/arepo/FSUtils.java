...
import static org.arepo.ASettings.SEP;
...
public class FSUtils {
    ...
    public static Map<String, Map<String, byte[]>> getAllFiles7(final Path currentPath, Set<String> ignoreFiles, Set<String> igroreDirs)throws IOException{
        var result = new LinkedHashMap<String, Map<String, byte[]>>();
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                if(attrs.isRegularFile()){
                    ...
                    if(!ignoreFiles.contains(relPath)){
                        ...
                        if(!founded){
                            result.get(ASettings.DEFAULT_FOLDER_ROOT_NAME).put(relPath, Files.readAllBytes(file));
                        }
                    }
                }
                return CONTINUE;
        ...
        for(var tmp  : alldirs.values()){
            result.put(tmp, new HashMap<String, byte[]>());
        }
        for(var dir : alldirs.entrySet()) {
            var currentDir = dir.getValue();
            //System.out.println("curr:"+currentDir);
            if(ASettings.DEFAULT_FOLDER_ROOT_NAME.equals(currentDir)){
                    ...
                    Files.walkFileTree(currentPath.resolve(currentDir), EnumSet.noneOf(FileVisitOption.class), Integer.MAX_VALUE, new SimpleFileVisitor<Path>() {
                        ...
                        public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                            if(attrs.isRegularFile()){
                                var relPath = currentPath.relativize(file).toString().replace("\\", SEP);
                                if(!ignoreFiles.contains(relPath)){
                                    result.get(currentDir).put(relPath, Files.readAllBytes(file));
                                }
                            }
                            return CONTINUE;
                        }
    ...
    public static Map<Integer, Map<String, Path>> getAllFiles4(final Path currentPath, Set<String> igroreDirs) throws IOException {
        ...
        FileVisitor<Path> rootVisitor = new SimpleFileVisitor<Path>() {
            ...
            public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
                if(attrs.isRegularFile()){
                    var relPath = currentPath.relativize(file).toString().replace("\\", SEP);
                    System.out.println("i:"+relPath.indexOf(SEP));
                    var index = relPath.indexOf(SEP);
                    if(index == -1){
