...
import static org.arepo.ASettings.FILE_TABLE;
...
public class TextModel2 {
    ...
    public void add(Entities.TextFiles notes){
        ...
        var sql = """
                INSERT INTO %s 
            (rootfolder, name, content, hash, parent, version, charset) 
            values (?, ?, ?, ?, ?, ?, ?);
                """;
        var id = query.update(sql.formatted(FILE_TABLE)).params(notes.rootfolder, notes.getName(), notes.getContent(), notes.getHash(), notes.getParent(), notes.getVersion(), notes.getCharset()).runFetchGenKeys(Mappers.singleLong()).generatedKeys().get(0);
        notes.setId(id);
    }
    ...
    public int insertFiles(List<Entities.TextFiles> textFiles) {
        var sql = """
                    INSERT INTO %s 
            (id, rootfolder, name, content, hash, parent, version, charset) 
            values (?,?,?,?,?,?,?,?);
                    """.formatted(FILE_TABLE);
        for(var notes : textFiles){
            query.update(sql).params(notes.getId(), notes.rootfolder, notes.getName(), notes.getContent(), notes.getHash(), notes.getParent(), notes.getVersion(), notes.getCharset()).run();
        }
        return 0;
    }
    ...
    public Set<String> getRootFolders(){
        var sql = "SELECT DISTINCT rootfolder FROM %s ;".formatted(FILE_TABLE);
        return query.select(sql).setResult(row -> row.getString("rootfolder"));
    }
}
